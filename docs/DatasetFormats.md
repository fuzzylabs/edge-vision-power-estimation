...
In this document, we explain the dataset format for the following datasets stored in the FuzzyLabs DagsHub repository.
...
A `raw_data` folder is created and uploaded to DagsHub repository as part of running the [Jetson power logging workflow](../jetson/power_logging/). The raw dataset is saved in the following format:
...
It contains an `idling_power.json` file that records average idle power and a folder for each model recording the benchmarking values for the model.

### Idle power

The average idle power is calculated and stored in an `idling_power.json` file. An example of this file is shown below. It saves two values: a timestamp when idle power measurement was completed and the calculated average power in microwatts.
...
`<model_name>_power_log.log`: This log file records the 3 values: timestamp, voltage and current measurements while the benchmarking experiment is being run in a separate process. An example of a saved power log file is shown below
...
Inside the `trt_profiling` folder for each model, we have two files `trt_engine_info.json` and `trt_layer_latency.json`.
...
`trt_layer_latency.json`: This JSON file saves a dictionary mapping layer name to a tuple of (`timestamp`, `latency`) for each iteration of benchmarking. An example of this file for the first layer of the Alexnet TensorRT model is shown below:
...
A `preprocessed_data` folder is created after running the [`map_power_to_layers.py`](../model_training/map_power_to_layers.py) script. It takes as input the raw data folder, and then maps power and runtime values for each layer and dumps this to a CSV file.
...
A `training_data` folder is created after running the [`convert_measurements.py`](../model_training/convert_measurements.py) script. It takes input the preprocessed data folder and splits the data into 3 CSV files according to layer types of interest.
...
An example of columns in [Alexnet dense CSV](https://dagshub.com/fuzzylabs/edge-vision-power-estimation/src/main/training_data/alexnet/dense.csv) are:
...
An example of columns in [Alexnet pooling CSV](https://dagshub.com/fuzzylabs/edge-vision-power-estimation/src/main/training_data/alexnet/pooling.csv) are:
...
An example of columns in Alexnet convolutional CSV are:
